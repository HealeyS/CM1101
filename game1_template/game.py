...
def normalise_input(user_input):
    ...
    'help'
    """
    #remove all punctuation, leading and trailing spaces from a string, and converts the string to lower case.
...
def display_room(room):
    ...
    print("\n%s\n\n%s\n" % (str(room["name"]).upper(), str(room["description"])))
...
def exit_leads_to(exits, direction):
    ...
    'Reception'
    """
    #returns name of room where the exit leads
...
def print_menu_line(direction, leads_to):
    ...
    """
    #displays direction and where it goes
...
def is_valid_exit(exits, user_input):
    ...
    True
    """
    #check whether player has chosen valid exit
    for x in exits:
        if user_input == x:
            return True
    return False
...
def menu(exits):
    ...
    while True:
        ...
        if is_valid_exit(exits, user_input[3:])  == True:
...
def move(exits, direction):
    ...
    False
    """
    #returns the room into which the player willmove if valid
